...
initWaitingGroup(struct NumBlock *numBlock,struct Queue* queue){
  numBlock->base=1;
  numBlock->seqNum=1;
  QueueInit(queue);
}
...
checkWaitingFull(struct NumBlock numBlock){
  return !(numBlock.seqNum<numBlock.base+MAXWAITINGNUM);
}

checkWaitingEmpty(struct NumBlock numBlock){
  return numBlock.seqNum==numBlock.base;
}

addToWaitingQueue(struct NumBlock *numBlock,struct Queue *queue,struct pkt save){
  numBlock->seqNum++;
  queue->add(queue,save);
}
...
void sendPkt2A(struct pkt *save){
  tolayer3(A, save);
}

resendWaitingQueue(struct Queue *queue){
  void (*funcPtr)(struct pkt *)=&sendPkt2A;
  queue->apply(queue,funcPtr);
}

// package message with a deep copy
struct pkt packageMsg(struct msg message,struct NumBlock block)
{
  struct pkt packet;
  packet.seqnum=block.seqNum;
  packet.acknum=1;
...
A_output(message)
  struct msg message;
{
  if(!checkWaitingFull(block_a)){
    return -1;
  }
  struct pkt save=packageMsg(message,block_a);
  int isEmpty=checkWaitingEmpty(block_a);
  addToWaitingQueue(&block_a,&waiting_queue,save);
  tolayer3(A, save);
  if(isEmpty){
  ...
  struct pkt packet;
{
  ...
  int seqNum=packet.seqnum+1;
  modifyWaitingQueue(&waiting_queue,&block_a,seqNum);
  if(checkWaitingEmpty(block_a)){
    stoptimer(A);
  }
  else{
    stoptimer(A);
...
A_timerinterrupt()
{
  resendWaitingQueue(&waiting_queue);
  starttimer(A,TIMEINCREMENT);
}  
...
A_init()
{
  initWaitingGroup(&block_a,&waiting_queue);
}
...
B_input(packet)
  struct pkt packet;
{
  // seqnum incorrect means wrong order or just broken package, send ack_pkt again
  if(packet.seqnum!=ack_pkt.seqnum+1||!checkRcv(packet)){
    tolayer3(B,ack_pkt);
    return;
  }
